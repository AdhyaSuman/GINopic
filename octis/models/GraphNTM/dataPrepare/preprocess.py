...
sys.path.append("../")
from octis.models.GraphNTM.settings import *
import re
import pandas as pd
import numpy as np
import os
...
def process_dataset(dataset='AG', STOPWORD=False, tr=None, va=None, ts=None,
                    tr_l=None, va_l=None, ts_l=None, octis_save=None):
    if STOPWORD:
        load_stopwords()
    if dataset == 'BNC':
        path = BNC_ADDR + '/raw/'
    ...
    elif dataset[:5] =='OCTIS':
        data = pd.DataFrame(tr, columns=['content'])
        data['train'] = 1
        if tr_l:
            data['label'] = tr_l
        else:
        ...
        if va:
            data2 = pd.DataFrame(va, columns=['content'])
            data2['train'] = -1
            if va_l:
                data2['label'] = va_l
            else:
        ...
        if ts:
            data3 = pd.DataFrame(ts, columns=['content'])
            data3['train'] = 0
            if ts_l:
                data3['label'] = ts_l
            else:
        ...
        # data.to_csv(octis_save+'/{}_data.csv'.format(), quoting=1, header=True, index=False)
...
def select_embedding(path, nw=300, STOPWORD=False):
    ...
    with open(GLOVE_ADDR + 'glove.840B.{}d.txt'.format(nw), 'rb') as fd:
        for l in fd:
            ...
            try:
                idx = word2id[word]
                weights_matrix[idx] = np.array(line[1:]).astype(np.float64)
                words_found[word] = 1
            except:
                continue
    idxlist = []
