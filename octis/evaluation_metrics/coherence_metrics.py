...
class TopKCoherence(AbstractMetric):
    def __init__(self, texts=None, topk=10, measure='c_npmi', topTopics=10):
        """
        Initialize metric
    ...
    def score(self, model_output):
        ...
        if self.topk > len(topics[0]):
            ...
            npmi = CoherenceModel(topics=topics, texts=self._texts, dictionary=self._dictionary,
                                  coherence=self.measure, processes=-1, topn=self.topk)
            npmis =  npmi.get_coherence_per_topic()
            npmis = np.asarray(npmis, dtype=np.float32)
            # print('npmis=', npmis)
            topK_npmis = npmis[np.argsort(npmis)[-self.topTopics:]]
            # print('topK_npmis=', topK_npmis)
            return topK_npmis.mean()
